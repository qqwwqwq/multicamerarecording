...
if __name__ == "__main__":
	path=r'/mnt/storage/buildwin/newfive'
	...
	for name in filenames:
		...
		if name[:-4] in fname2:
			continue
		video_filename = '/mnt/storage/buildwin/newfive/'+name
		print(video_filename)
		st = video_filename[30:-4]
		print(st)
		...
		while playback.isOpened():
			...
			if num_bodies == 2:
				if len(cenp(center1)) == 0:
					...
					center1 = body_handle
					current1 = body_frame.get_body(0).handle()
				if len(cenp(center2)) == 0:
					body_handle = body_frame.get_body(1).handle()
					if cenp(center1) == cenp(body_handle):
						print("ssssssssssssssss")
						body_handle = body_frame.get_body(0).handle()
					center2 = body_handle
					print(cenp(center2), cenp(center1))
					current1 = body_frame.get_body(0).handle()
					current2 = body_frame.get_body(1).handle()
				else:
					current1 = body_frame.get_body(0).handle()
					current2 = body_frame.get_body(1).handle()
			for body_id in range(num_bodies):
				...
				if num_bodies == 2 and dis(cenp(center1), cenp(current1)) > dis(cenp(center1), cenp(current2)):
					print("switch")
				...
				if body_id == 1:
					for jointID, joint in enumerate(current2.skeleton.joints):
						if jointID == 1:
							ref_x_B = joint.position.xyz.x
							ref_y_B = joint.position.xyz.y
							ref_z_B = joint.position.xyz.z
					for jointID, joint in enumerate(current2.skeleton.joints):
						# print([joint.position.xyz.x,joint.position.xyz.y,joint.position.xyz.z],jointID)
						joint_2d = playback_calibration.convert_3d_to_2d(joint.position, K4A_CALIBRATION_TYPE_DEPTH,
																		 K4A_CALIBRATION_TYPE_COLOR)
						# print(joint_2d.xy.x,joint_2d.xy.y)
						headone_B.append("x" + str(jointID))
						headone_B.append("y" + str(jointID))
						headone_B.append("z" + str(jointID))
						headone_B.append("p" + str(jointID))
						headone_m_B.append("x" + str(jointID))
						headone_m_B.append("y" + str(jointID))
						headone_m_B.append("z" + str(jointID))
						headone_m_B.append("p" + str(jointID))
						headone_2d_B.append("x" + str(jointID))
						headone_2d_B.append("y" + str(jointID))
						headone_2d_B.append("p" + str(jointID))
						dataone_B.append(joint.position.xyz.x)
						dataone_B.append(joint.position.xyz.y)
						dataone_B.append(joint.position.xyz.z)
						dataone_B.append(joint.confidence_level)
						dataone_m_B.append((joint.position.xyz.x) / 1000)
						dataone_m_B.append((joint.position.xyz.y) / 1000)
						dataone_m_B.append((joint.position.xyz.z) / 1000)
						dataone_m_B.append(joint.confidence_level)
						dataone_2d_B.append(joint_2d.xy.x)
						dataone_2d_B.append(joint_2d.xy.y)
						dataone_2d_B.append(joint.confidence_level)
			center1 = current1
			center2 = current2
			...
			if tag_B == 0:
				header_B = headone_B
				header_m_B = headone_m_B
				header_2d_B = headone_2d_B
				tag_B = 1
			datas.append(dataone)
			datas_m.append(dataone_m)
			datas_2d.append(dataone_2d)
			datas_B.append(dataone_B)
			datas_m_B.append(dataone_m_B)
			datas_2d_B.append(dataone_2d_B)
			# print(datas)
		...
		with open("/mnt/storage/buildwin/newfive_csv_3d/" + "3d_" + st + "_B2.csv", 'w', encoding='UTF8', newline='') as f:
			writer = csv.writer(f)
		...
		with open("/mnt/storage/buildwin/newfive_csv_3d_m/" + "3d_" + st + "_B2.csv", 'w', encoding='UTF8', newline='') as f:
			writer = csv.writer(f)

			# 写入头
			writer.writerow(header_m_B)

			# 写入数据
			writer.writerows(datas_m_B)
		...
		with open("/mnt/storage/buildwin/newfive_csv_2d/" + "2d_" + st + "_B2.csv", 'w', encoding='UTF8', newline='') as f:
			writer = csv.writer(f)

			# 写入头
			writer.writerow(header_2d_B)

			# 写入数据
			writer.writerows(datas_2d_B)
		playback.close()
	# print("done")